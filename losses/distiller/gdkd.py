...
class GDKD(nn.Module):
    ...
    def forward_flatten(self, y_s, y_t, target):
        loss, p_s, p_t, self.train_info = self._gdkd_loss(y_s, y_t, target)
    ...
    def _gdkd_loss(self, y_s, y_t, target):
        train_info = {}
        ...
        train_info.update(dict(num_ignore=target.shape[0]-num_valid.detach()))
